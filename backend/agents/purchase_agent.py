...
Validates: supplier present, invoice number, 3-way match (PO→GRN→Invoice), duplicate invoices.
"""
...
def run(period_id: int, db: Session) -> AgentResult:
    files = db.query(FileModel).filter(
        FileModel.period_id == period_id, FileModel.file_type == "purchase"
    ).all()
    if not files:
        return AgentResult("Procure-to-Pay", "skipped", summary="No purchase files uploaded")
    ...
    for f in files:
        ...
        results = validate_records(rec_dicts, "purchase", period_id)
        for res in results:
    ...
    return AgentResult(
        ...
        summary=f"Validated {len(all_records)} purchase records. {len(exceptions)} exceptions."
    )
