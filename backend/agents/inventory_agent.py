"""
Inventory-to-COGS Agent
Validates: negative stock, waste rate, COGS support, SKU existence.
"""
from sqlalchemy.orm import Session
from backend.database import BusinessRecord, ValidationResult, JournalEntry, JournalLine, File as FileModel
from backend.services.validation_engine import validate_records
...
def run(period_id: int, db: Session) -> AgentResult:
    files = db.query(FileModel).filter(
        FileModel.period_id == period_id, FileModel.file_type == "inventory"
    ).all()
    if not files:
        return AgentResult("Inventory-to-COGS", "skipped", summary="No inventory files uploaded")
    ...
    for f in files:
        ...
        results = validate_records(rec_dicts, "inventory", period_id)
        for res in results:
        ...
        for entry in entries:
            je = JournalEntry(period_id=period_id, source_record_id=entry["source_record_id"],
    ...
    return AgentResult(
        "Inventory-to-COGS", status,
        records_checked=len(all_records),
        exceptions=exceptions[:10],
        journal_entries=entries_created,
        summary=f"Validated {len(all_records)} inventory records. {len(exceptions)} exceptions."
    )
